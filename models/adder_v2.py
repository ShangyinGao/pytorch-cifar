...
def adder2d_function(X, W, stride=1, padding=0, p=1):
...
class adder2d(nn.Module):
    ...
    def forward(self, x):
        output = adder2d_function(x,self.adder, self.stride, self.padding, p=1)
        if self.bias:
            output += self.b.unsqueeze(0).unsqueeze(2).unsqueeze(3)

        output = output.detach()

        output_2 = adder2d_function(x,self.adder, self.stride, self.padding, p=2)
        output_2.data = output.data
        # pdb.set_trace()
